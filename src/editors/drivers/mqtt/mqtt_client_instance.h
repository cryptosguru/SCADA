...
#ifndef MQTT_CLIENT__INSTANCE
#define MQTT_CLIENT__INSTANCE
...
class MQTT_client_DriverThread;

class MQTT_CLIENT_DRV MQTT_client_Instance : public DriverInstance 
{
	Q_OBJECT
	...
	public:
	MQTT_client_DriverThread *pConnect;
	unsigned int msg_sent_in_control_direction;
	//
	MQTT_client_Instance(Driver *parent, const QString &name, int instance_id) : 
	DriverInstance(parent,name),fFail(0), Countdown(1), pConnect(NULL),Config_db(NULL),
	State(STATE_RESET),InTick(0),Retry(0),Sp(0),MqttItems(1), Values(NULL),
	ParentDriver(parent),msg_sent_in_control_direction(0), instanceID(instance_id)
	{
		IT_IT("MQTT_client_Instance::MQTT_client_Instance");
