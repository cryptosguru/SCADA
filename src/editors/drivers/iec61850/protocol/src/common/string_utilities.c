...
StringUtils_digitsToInt(const char* digits, int count)
{
	...
	while (i < count) {
		...
		digitValue = StringUtils_digitToInt(*(digits + i));
...
StringUtils_createBufferFromHexString(char* hexString, uint8_t* buffer)
{
    ...
    for (i = 0; i < (hexStringLen/2); i++) {
        high = toInt(hexString[i * 2]);
        if (high == -1) return -1;

        low = toInt(hexString[(i * 2) + 1]);
        if (low == -1) return -1;
...
StringUtils_sortList(LinkedList list)
{
	...
	while (selectedElement != NULL) {
		...
		str1 = (char*) LinkedList_getData(selectedElement);

		prevElement = sortedList;
		...
		while (true) {
			...
			str2 = (char*) LinkedList_getData(prevElement->next);
