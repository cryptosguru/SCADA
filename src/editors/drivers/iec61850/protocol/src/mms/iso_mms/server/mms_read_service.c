		...
		MmsVariableSpecification* namedVariable)
{
	if (mmsServer_isIndexAccess(alternateAccess))
	{
		...
		arrayValue = mmsServer_getValue(connection->server, domain, itemId, connection);
...
encodeVariableAccessSpecification(VarAccessSpec* accessSpec, uint8_t* buffer, int bufPos, bool encode)
{
	...
	uint32_t specificityLength;
	uint32_t variableListNameLength;
	uint32_t varAccessSpecLength;
        ...
        VarAccessSpec* accessSpec)
{
	...
	LinkedList value;
	uint32_t listOfAccessResultsLength;
	uint32_t confirmedServiceResponseContentLength;
	uint32_t confirmedServiceResponseLength;
	uint32_t invokeIdSize;
	uint32_t confirmedResponseContentSize;
	uint32_t mmsPduSize;
	uint8_t* buffer;
	int bufPos;
    ...
    mmsPduSize = 1 + BerEncoder_determineLengthSize(confirmedResponseContentSize) +
