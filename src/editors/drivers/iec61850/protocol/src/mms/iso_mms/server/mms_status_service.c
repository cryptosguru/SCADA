        ...
        ByteBuffer* response)
{
	...
	uint8_t* buffer;
	uint32_t invokeIdLength;
	MmsServer mmsServer;
	uint32_t vmdPhysicalStatusLength;
	uint32_t vmdLogicalStatusLength;
	uint32_t statusLength;
	uint32_t statusResponseLength;
    ...
    vmdPhysicalStatusLength = BerEncoder_UInt32determineEncodedSize((uint32_t) mmsServer->vmdPhysicalStatus);
    vmdLogicalStatusLength = BerEncoder_UInt32determineEncodedSize((uint32_t) mmsServer->vmdLogicalStatus);

    statusLength = 2 + vmdPhysicalStatusLength + 2 + vmdLogicalStatusLength;

    statusResponseLength = invokeIdLength + 2 + 1 + BerEncoder_determineLengthSize(statusLength) + statusLength;
