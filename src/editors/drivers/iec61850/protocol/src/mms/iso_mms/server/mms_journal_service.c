...
entryDataCallback (void* parameter, const char* dataRef, uint8_t* data, int dataSize, uint8_t reasonCode, bool moreFollow)
{
    ...
    if (moreFollow) {
		...
		uint32_t reasonCodeValueLen;
		uint32_t reasonCodeContentLen;
		uint32_t secondVariableLen;
		uint32_t totalLen;
        ...
        reasonCodeValueLen = MmsValue_encodeMmsData(&reasonCodeValue, NULL, 0, false);

        reasonCodeContentLen = reasonCodeValueLen + 2 + 12;

        secondVariableLen = 1 + BerEncoder_determineLengthSize(reasonCodeContentLen)
                + reasonCodeContentLen;

        totalLen = firstVariableLen + secondVariableLen;
        ...
        entryHeaderLength =  headerBufPos - encoder->currentEntryBufPos;
        ...
        ByteBuffer* response)
{
    ...
    uint32_t listOfEntriesLen;
    uint32_t moreFollowsLen;
	uint32_t readJournalLen;
    ...
    mmsDomain = MmsDevice_getDomain(mmsDevice, domainId);
    ...
    logStorage = mmsJournal->logStorage;
    ...
    dataSize = encoder.bufPos - RESERVED_SPACE_FOR_HEADER;

    invokeIdSize = BerEncoder_UInt32determineEncodedSize((uint32_t) invokeId) + 2;
    listOfEntriesLen = 1 + BerEncoder_determineLengthSize(dataSize) + dataSize;
    ...
    readJournalLen = 2 + BerEncoder_determineLengthSize(listOfEntriesLen + moreFollowsLen) +
             (listOfEntriesLen + moreFollowsLen);

    confirmedResponsePDUSize = readJournalLen + invokeIdSize;
