...
MmsValue_getNumberOfSetBits(const MmsValue* self)
{
    ...
    for (i = 0; i < byteSize; i++) {
        ...
        currentByte = self->value.bitString.buf[i];
...
{
    uint32_t timeval32 = (timeval / 1000L);
...
MmsValue_getUtcTimeInMs(const MmsValue* self)
{
    ...
    msVal = (timeval32 * 1000L) + remainder;
...
MmsValue_getUtcTimeInMsWithUs(const MmsValue* self, uint32_t* usec)
{
    ...
    remainder = fractionOfSecond * 1000000UL / 0x1000000UL; // in usec

    msVal = (timeval32 * 1000L) + (remainder / 1000L);

    if (usec != NULL)
        *usec = remainder % 1000L;
...
MmsValue_cloneToBuffer(const MmsValue* self, uint8_t* destinationAddress)
{
    ...
    case MMS_UNSIGNED:
        {
			Asn1PrimitiveValue* newAsn1Value;
            newValue->value.integer = (Asn1PrimitiveValue*) destinationAddress;
            newAsn1Value = (Asn1PrimitiveValue*) destinationAddress;
...
MmsValue_clone(const MmsValue* self)
{
    ...
    newValue = (MmsValue*) GLOBAL_CALLOC(1, sizeof(MmsValue));
...
MmsValue_setBinaryTime(MmsValue* self, uint64_t timestamp)
{
    ...
    if (timestamp > 441763200000L)
        mmsTime = timestamp - (441763200000L);
    else
        mmsTime = 0;

    binaryTimeBuf = self->value.binaryTime.buf;

    if (self->value.binaryTime.size == 6) {
        uint16_t daysDiff = mmsTime / (86400000L);
    ...
    msSinceMidnight = mmsTime % (86400000L);
...
MmsValue_getBinaryTimeAsUtcMs(const MmsValue* self)
{
    ...
    if (self->value.binaryTime.size == 6) {
        ...
        mmsTime = daysDiff * (86400000L);

        timestamp = mmsTime + (441763200000L);
    }
    ...
    case MMS_ARRAY:
        {
            ...
            arraySize = MmsValue_getArraySize(self);
