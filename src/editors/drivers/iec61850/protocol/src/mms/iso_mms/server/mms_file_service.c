        ...
        char* filename, MmsFileReadStateMachine* frsm)
{
	...
	char gtString[30];
	uint32_t fileAttributesSize;
	uint32_t invokeIdSize;
	uint32_t frsmIdSize;
	uint32_t openFileResponseSize;
    ...
    fileAttributesSize = encodeFileAttributes(0xa1, frsm->fileSize, gtString, NULL, 0) + 2;

    invokeIdSize = BerEncoder_UInt32determineEncodedSize(invokeId) + 2;

    frsmIdSize = BerEncoder_UInt32determineEncodedSize(frsm->frsmId) + 2;

    openFileResponseSize = fileAttributesSize + frsmIdSize;

    confirmedResponsePDUSize = invokeIdSize + 2 + BerEncoder_determineLengthSize(openFileResponseSize)
...
//#endif
        ...
        task = MmsServer_getObtainFileTask(connection->server);
        ...
        if (task != NULL) {
            ...
            if (fileHandle == NULL) {
                ...
                request = MmsServer_reserveTransmitBuffer(connection->server);
        ...
        ByteBuffer* response,  MmsFileReadStateMachine* frsm)
{
     ...
     confirmedResponsePDUSize = invokeIdSize + 2 + BerEncoder_determineLengthSize(fileReadResponseSize)
...
addFileEntriesToResponse(const char* basepath, uint8_t* buffer, int bufPos, int maxBufSize, char* directoryName, char** continueAfterFileName, bool* moreFollows)
{
    ...
    if (directory != NULL) {
        ...
        if (*continueAfterFileName != NULL) {
            ...
            uint32_t fileSize;
			int fileAttributesSize;
			int filenameSize;
			int dirEntrySize;
			int overallEntrySize;
			int bufferSpaceLeft;
            ...
            if (getFileInfo(basepath, directoryName, &fileSize, &msTime)) {
                ...
                fileAttributesSize = encodeFileAttributes(0xa1, fileSize, gtString, NULL, 0);

                filenameSize = encodeFileSpecification(0xa0, directoryName, NULL, 0);

                dirEntrySize = 2 + fileAttributesSize + filenameSize;

                overallEntrySize = 1 + BerEncoder_determineLengthSize(dirEntrySize) + dirEntrySize;

                bufferSpaceLeft = maxBufSize - bufPos;
...
createFileDirectoryResponse(const char* basepath, uint32_t invokeId, ByteBuffer* response, int maxPduSize, char* directoryName, char* continueAfterFileName)
{
    ...
    int tempEncoded = 0;
	uint32_t invokeIdSize;
	uint32_t listOfDirectoryEntrySeqSize;
	uint32_t listOfDirectoryEntrySize;
	uint32_t fileDirectoryResponseSize;
	uint32_t confirmedResponsePDUSize;
	int bufPos;
    ...
    listOfDirectoryEntrySeqSize = tempEncoded;

    listOfDirectoryEntrySize = 1 + BerEncoder_determineLengthSize(tempEncoded) + tempEncoded;

    fileDirectoryResponseSize = 1 + BerEncoder_determineLengthSize(listOfDirectoryEntrySize)
            + listOfDirectoryEntrySize;

    if (moreFollows)
        fileDirectoryResponseSize += 3; /* for moreFollows */

    confirmedResponsePDUSize = invokeIdSize + 2 + BerEncoder_determineLengthSize(fileDirectoryResponseSize)
