...
handleLastApplErrorMessage(IedConnection self, MmsValue* lastApplError)
{
    ...
    cntrlObj = MmsValue_getElement(lastApplError, 0);
    error = MmsValue_getElement(lastApplError, 1);
    //MmsValue* origin = MmsValue_getElement(lastApplError, 2);
    ctlNum = MmsValue_getElement(lastApplError, 3);
    addCause = MmsValue_getElement(lastApplError, 4);
    if (DEBUG_IED_CLIENT)
    ...
    control = LinkedList_getNext(self->clientControls);
    while (control != NULL) {
...
IedConnection_getDeviceModelFromServer(IedConnection self, IedClientError* error)
{
	...
	LinkedList logicalDeviceNames;
	LinkedList logicalDevice;
	LinkedList logicalDevices;

    *error = IED_ERROR_OK;

    logicalDeviceNames = MmsConnection_getDomainNames(self->connection, &mmsError);
    ...
    if (logicalDeviceNames != NULL) {
        ...
        logicalDevice = LinkedList_getNext(logicalDeviceNames);

        logicalDevices = LinkedList_create();
        ...
        const char* logicalDeviceName)
{
	...
	*error = IED_ERROR_OK;
    ...
    logicalDevice = LinkedList_getNext(self->logicalDevices);
        ...
        const char* logicalNodeName)
{
    ...
    logs = LinkedList_create();

    journal = LinkedList_getNext(journals);
        ...
        const char* logicalNodeName)
{
    ...
    lnDataSets = LinkedList_create();

    dataSet = LinkedList_getNext(dataSets);
        ...
        const char* logicalNodeReference, ACSIClass acsiClass)
{
    ...
    *ldSep = 0;
    ...
    logicalDeviceName = lnRefCopy;

    logicalNodeName = ldSep + 1;
    ...
    device = LinkedList_getNext(self->logicalDevices);
        ...
        const char* logicalNodeReference)
{
    ...
    *ldSep = 0;
    ...
    variable = LinkedList_getNext(ld->variables);
        ...
        const char* dataReference, bool withFc)
{
	...
	char* logicalDeviceName;
    char* logicalNodeName;
    char* logicalNodeNameEnd;
    int logicalNodeNameLen;
    char* dataNamePart;
    int dataNamePartLen;
    LinkedList device;
    bool deviceFound;
    ...
    *error = IED_ERROR_OK;
    ...
    ldSep = strchr(dataRefCopy, '/');

    *ldSep = 0;

    logicalDeviceName = dataRefCopy;

    logicalNodeName = ldSep + 1;

    logicalNodeNameEnd = strchr(logicalNodeName, '.');
    ...
    logicalNodeNameLen = logicalNodeNameEnd - logicalNodeName;

    dataNamePart = logicalNodeNameEnd + 1;

    dataNamePartLen = strlen(dataNamePart);
        ...
        if (fcPos != NULL) {
            ...
            if (logicalNodeNameLen == lnNameLen) {
                ...
                if (memcmp(variableName, logicalNodeName, lnNameLen) == 0) {
                    ...
                    remainingPart = fcEnd + 1;

                    remainingLen = strlen(remainingPart);
                ...
                if (memcmp(variableName, logicalNodeName, lnNameLen) == 0) {
                    ...
                    if (remainingPart[dataNamePartLen] == '$') {
                        ...
                        if (memcmp(dataNamePart, remainingPart, dataNamePartLen) == 0) {
                            ...
                            elementNameLen = strlen(subElementName);

                            elementName = (char*) GLOBAL_MALLOC(elementNameLen + 1);
                            memcpy(elementName, subElementName, elementNameLen);
        ...
        LinkedList /* <char*> */dataSetElements)
{
    ...
    if (dataSetReference[0] != '@') {
        ...
        if ((dataSetReference[0] == '/') || (strchr(dataSetReference, '/') == NULL)) {
            ...
            domainIdLength = strlen(domainId);
            ...
            itemIdRef = StringUtils_copyStringToBuffer(dataSetReference + domainIdLength + 1, itemIdBuffer);
...
IedConnection_deleteDataSet(IedConnection self, IedClientError* error, const char* dataSetReference)
{
    ...
    if (dataSetReference[0] != '@') {
        if ((dataSetReference[0] == '/') || (strchr(dataSetReference, '/') == NULL)) {
            ...
            itemIdString = dataSetReference + strlen(domainId) + 1;
...
IedConnection_getDataSetDirectory(IedConnection self, IedClientError* error, const char* dataSetReference, bool* isDeletable)
{
    ...
    if (dataSetReference[0] != '@') {
        if ((dataSetReference[0] == '/') || (strchr(dataSetReference, '/') == NULL)) {
            ...
            itemIdRef = dataSetReference + strlen(domainId) + 1;
            ...
            itemIdRefInBuffer = StringUtils_copyStringToBuffer(itemIdRef, itemIdBuffer);
        ...
        ClientDataSet dataSet)
{
    ...
    if (dataSetReference[0] != '@') {
        ...
        if ((dataSetReference[0] == '/') || (strchr(dataSetReference, '/') == NULL)) {
            ...
            itemIdRefOrig = dataSetReference + strlen(domainId) + 1;
            ...
            itemIdRef = StringUtils_copyStringToBuffer(itemIdRefOrig, itemIdBuffer);
        ...
        uint64_t startTime, uint64_t endTime, bool* moreFollows)
{
    ...
    char* logDomain;
    char* logName;
	MmsValue* startTimeMms;
	MmsValue* endTimeMms;
	LinkedList journalEntries;

    strncpy(logRef, logReference, 129);

    logDomain = logRef;
    logName = strchr(logRef, '/');
    ...
    if (logName != NULL) {
        ...
        startTimeMms = MmsValue_newBinaryTime(false);
        MmsValue_setBinaryTime(startTimeMms, startTime);

        endTimeMms = MmsValue_newBinaryTime(false);
        MmsValue_setBinaryTime(endTimeMms, endTime);

        journalEntries = MmsConnection_readJournalTimeRange(self->connection, &mmsError, logDomain, logName,
        ...
        MmsValue* entryID, uint64_t timeStamp, bool* moreFollows)
{
    ...
    if (logName != NULL) {
        ...
        timeStampMms = MmsValue_newBinaryTime(false);
        MmsValue_setBinaryTime(timeStampMms, timeStamp);

        journalEntries = MmsConnection_readJournalStartAfter(self->connection, &mmsError, logDomain, logName,
