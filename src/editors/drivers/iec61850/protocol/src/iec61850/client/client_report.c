...
IedConnection_uninstallReportHandler(IedConnection self, const char* rcbReference)
{
    ...
    report = lookupReportHandler(self, rcbReference);
...
IedConnection_triggerGIReport(IedConnection self, IedClientError* error, const char* rcbReference)
{
    ...
    mmsCon = IedConnection_getMmsConnection(self);

    gi = MmsValue_newBoolean(true);
...
private_IedConnection_handleReport(IedConnection self, MmsValue* value)
{
	...
	MmsValue* optFlds;
	int inclusionIndex;
	MmsValue* dataSetName;
	const char* dataSetNameStr;
	MmsValue* moreSegmentsFollow;
	MmsValue* inclusion;
	int dataSetSize;
   ...
   if (DEBUG_IED_CLIENT)
    ...
    if (MmsValue_getBitStringBit(optFlds, 4) == true) {
        ...
        dataSetName = MmsValue_getElement(value, inclusionIndex);
        ...
        dataSetNameStr = MmsValue_toString(dataSetName);
    ...
    if (MmsValue_getBitStringBit(optFlds, 9) == true) {
        ...
        moreSegmentsFollow = MmsValue_getElement(value, inclusionIndex);
        inclusionIndex++;
