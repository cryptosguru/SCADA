...
createNamedVariableFromDataAttribute(DataAttribute* attribute)
{
    ...
    if (attribute->firstChild != NULL) {
        ...
        componentCount = ModelNode_getChildCount((ModelNode*) attribute);

        namedVariable->typeSpec.structure.elements = (MmsVariableSpecification**) GLOBAL_CALLOC(componentCount,
                sizeof(MmsVariableSpecification*));

        subDataAttribute = (DataAttribute*) attribute->firstChild;
        ...
        FunctionalConstraint fc)
{
    ...
    elementCount = countChildrenWithFc(dataObject, fc);
    ...
    i = 0;
    component = dataObject->firstChild;
...
unselectEditSettingGroup(SettingGroup* settingGroup)
{
    ...
    editSg = MmsValue_getElement(settingGroup->sgcbMmsValues, 2);
    MmsValue_setUint8(editSg, 0U);
    resvTms = MmsValue_getElement(settingGroup->sgcbMmsValues, 5);
...
MmsMapping_configureSettingGroups(MmsMapping* self)
{
    ...
    while (settingGroupElement != NULL) {
        ...
        if (values != NULL) {
            ...
            numOfSg = MmsValue_getElement(values, 0);
            actSg = MmsValue_getElement(values, 1);
            resvTms = MmsValue_getElement(values, 5);
...
MmsMapping_changeActiveSettingGroup(MmsMapping* self, SettingGroupControlBlock* sgcb, uint8_t newActiveSg)
{
    ...
    if (sg != NULL) {
        if (newActiveSg > 0 && newActiveSg <= sgcb->numOfSGs) {
            ...
            actSg = MmsValue_getElement(sg->sgcbMmsValues, 1);
            lActTm = MmsValue_getElement(sg->sgcbMmsValues, 4);
        ...
        LogicalNode* logicalNode)
{
	...
	int componentCount;
	SettingGroupControlBlock* sgControlBlock;
    ...
    componentCount = determineLogicalNodeComponentCount(logicalNode);
    ...
    if (sgControlBlock != NULL) {
        ...
        settingGroup = (SettingGroup*) GLOBAL_CALLOC(1, sizeof(SettingGroup));
...
#if (CONFIG_IEC61850_REPORT_SERVICE == 1)
    brcbCount = countReportControlBlocksForLogicalNode(self, logicalNode,
    true);
    ...
    urcbCount = countReportControlBlocksForLogicalNode(self, logicalNode,
    false);
    ...
    lcbCount = countLogControlBlocksForLogicalNode(self, logicalNode);
    ...
    gseCount = countGSEControlBlocksForLogicalNode(self, logicalNode);
    ...
    msvcbCount = countSVControlBlocksForLogicalNode(self, logicalNode, false);
    ...
    usvcbCount = countSVControlBlocksForLogicalNode(self, logicalNode, true);
    ...
    if (self->iedServer->logServiceEnabled) {
        ...
        while (log != NULL) {
            ...
            if (nameLength > 63) {
                ...
                logInstance = LogInstance_create(log->parent, log->name);
...
createDataSets(MmsDevice* mmsDevice, IedModel* iedModel)
{
    ...
    while (dataset != NULL) {
        ...
        dataSetDomain = MmsDevice_getDomain(mmsDevice, domainName);
        ...
        varList = MmsNamedVariableList_create(dataSetDomain, dataset->name, false);

        dataSetEntry = dataset->fcdas;
...
createMmsModelFromIedModel(MmsMapping* self, IedModel* iedModel)
{
    ...
    if (iedModel->firstChild != NULL) {
        ...
        iedDeviceCount = IedModel_getLogicalDeviceCount(iedModel);
        ...
        MmsValue* value)
{
    ...
    *separator = 0;
    ...
    mmsGCB = lookupGCB(self, domain, lnName, objectName);
...
checkIfValueBelongsToModelNode(DataAttribute* dataAttribute, MmsValue* value, MmsValue* newValue)
{
    ...
    child = (DataAttribute*) dataAttribute->firstChild;
        ...
        char* variableId, MmsValue* value, MmsServerConnection connection)
{
	...
	int lnNameLength;
	FunctionalConstraint fc;
	LinkedList writeHandlerListElement;
	DataAttribute* da;
    ...
    if (isReportControlBlock(separator)) {
        ...
        nextElement = self->reportControls;
					...
					if ((val > 0) && (val <= sg->sgcb->numOfSGs)) {
						...
						if (sg->editSgChangedHandler != NULL) {
									...
									(uint8_t) val, (ClientConnection) connection))
							{
								...
								editSg = MmsValue_getElement(sg->sgcbMmsValues, 2);
								resvTms = MmsValue_getElement(sg->sgcbMmsValues, 5);
    ...
    fc = getFunctionalConstraintForWritableNode(self, separator);
    ...
    if (fc != IEC61850_FC_NONE) {
        ...
        if (cachedValue != NULL) {
            ...
            nodeAccessPolicy = getAccessPolicyForFC(self, fc);
            ...
            writeHandlerListElement = LinkedList_getNext(self->attributeAccessHandlers);
            ...
            da = IedModel_lookupDataAttributeByMmsValue(self->model, cachedValue);
                    ...
                    fc == IEC61850_FC_LG || fc == IEC61850_FC_GO)
                {
                    ...
                    if (ln != NULL) {
                        ...
                        if (doStart != NULL) {
                            ...
                            dobj = ModelNode_getChild((ModelNode*) ln, str);
                            ...
                            if (dobj != NULL) {
                                ...
                                if (dobj->modelType == DataObjectModelType) {
                                    ...
                                    return self->readAccessHandler(ld, ln, (DataObject*) dobj, fc, clientConnection, self->readAccessHandlerParameter);
                                }
                            }
                        }
                    }
        ...
        char* listName, MmsServerConnection connection)
{
    ...
    if (create) {
        if (listType == MMS_DOMAIN_SPECIFIC) {
            ...
            ld = IedModel_getDevice(model, domain->domainName);
        ...
        FunctionalConstraint fc, char* buffer)
{
    ...
    namePartLength = objRefLength - i - 1;
...
MmsMapping_startEventWorkerThread(MmsMapping* self)
{
    ...
    thread = Thread_create((ThreadExecutionFunction) eventWorkerThread, self, false);
...
MmsMapping_createDataSetByNamedVariableList(MmsMapping* self, MmsNamedVariableList variableList)
{
    ...
    element = LinkedList_getNext(variableList->listOfVariables);

    lastDataSetEntry = NULL;
...
MmsMapping_getDomainSpecificVariableList(MmsMapping* self, const char* variableListReference)
{
    ...
    separator = strchr(variableListReferenceCopy, '/');

    if (separator == NULL)
        return NULL;

    domainName = variableListReferenceCopy;

    variableListName = separator + 1;
    ...
    *separator = 0;
    ...
    variableList = MmsDomain_getNamedVariableList(domain, variableListName);
...
MmsMapping_ObjectReferenceToVariableAccessSpec(char* objectReference)
{
    ...
    fcStart = strchr(objectReference, '[');

    if (fcStart == NULL) /* no FC present */
        return NULL;

    fcEnd = strchr(fcStart, ']');
    ...
    fc = FunctionalConstraint_fromString(fcStart + 1);
    ...
    indexBrace = strchr(domainIdEnd, '(');

    itemIdEnd = indexBrace;

    if (itemIdEnd == NULL)
        itemIdEnd = strchr(domainIdEnd, '[');

    objRefLen = strlen(objectReference);
    ...
    if (itemIdEnd != NULL) {
        ...
        if (indexBrace != NULL) {
            ...
            componentNameLen = objRefLen - ((indexEnd + 2) - objectReference) - 4;
...
MmsMapping_varAccessSpecToObjectReference(MmsVariableAccessSpecification* varAccessSpec)
{
    ...
    newStringLen = (domainIdLen + 1) + (itemIdLen - 2) + arrayIndexLen + 4 /* for FC */+ componentPartLen + 1;

    newString = (char*) GLOBAL_MALLOC(newStringLen);
