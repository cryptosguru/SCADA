...
createControlObjects(IedServer self, MmsDomain* domain, char* lnName, MmsVariableSpecification* typeSpec, char* namePrefix)
{
    ...
    if (typeSpec->type == MMS_STRUCTURE) {
        ...
        for (i = 0; i < coCount; i++) {
            ...
            int sBOwIndex = 0;
            int operIndex = 0;
			MmsVariableSpecification* coSpec;
            ...
            coSpec = typeSpec->typeSpec.structure.elements[i];
            ...
            if (coSpec->type == MMS_STRUCTURE) {
                ...
                if (operSpec) {
                    ...
                    controlObject = ControlObject_create(self, domain, lnName, objectName, operSpec);

                    if (controlObject == NULL)
                        goto exit_function;

                    structure = MmsValue_newDefaultValue(coSpec);
...
createMmsServerCache(IedServer self)
{
    ...
    for (domain = 0; domain < self->mmsDevice->domainCount; domain++) {
        ...
        for (i = 0; i < logicalDevice->namedVariablesCount; i++) {
            ...
            fcCount = logicalDevice->namedVariables[i]->typeSpec.structure.elementCount;
            ...
            for (j = 0; j < fcCount; j++) {
                   ...
                   )
                {
					...
					MmsValue* defaultValue;

                    StringUtils_createStringInBuffer(variableName, 3, lnName, "$", fcName);

                    defaultValue = MmsValue_newDefaultValue(fcSpec);
        ...
        char* objectReference, int position)
{
    ...
    childPosition = strlen(objectReference);
        ...
        char* objectReference, int position)
{
    ...
    childNode = dataObject->firstChild;
...
installDefaultValuesInCache(IedServer self)
{
    ...
    while (logicalDevice != NULL) {
        ...
        nodeReference = objectReference + strlen(objectReference);
        ...
        while (logicalNode != NULL) {
            ...
            refPosition = strlen(objectReference);
    ...
    if (iedNameLength <= 64) {
        ...
        while (dataSet != NULL) {
            ...
            while (dataSetEntry != NULL) {
                ...
                value = MmsServer_getValueFromCache(self->mmsServer, domain, dataSetEntry->variableName);
...
//    Reporting_processReportEventsAfterUnlock(self->mmsMapping); apa---
...
lookupControlObject(IedServer self, DataObject* node)
{
    ...
    *separator = 0;
    ...
    domain = MmsDevice_getDomain(self->mmsDevice, objectReference);
    ...
    *separator = 0;
    ...
    controlObject = MmsMapping_getControlObject(self->mmsMapping, domain,
            lnName, objectName);
...
IedServer_updateFloatAttributeValue(IedServer self, DataAttribute* dataAttribute, float value)
{
    ...
    currentValue = MmsValue_toFloat(dataAttribute->mmsValue);
...
IedServer_updateInt32AttributeValue(IedServer self, DataAttribute* dataAttribute, int32_t value)
{
    ...
    currentValue = MmsValue_toInt32(dataAttribute->mmsValue);
...
IedServer_updateInt64AttributeValue(IedServer self, DataAttribute* dataAttribute, int64_t value)
{
    ...
    currentValue = MmsValue_toInt64(dataAttribute->mmsValue);
...
IedServer_updateUnsignedAttributeValue(IedServer self, DataAttribute* dataAttribute, uint32_t value)
{
    ...
    currentValue = MmsValue_toUint32(dataAttribute->mmsValue);
...
IedServer_updateBitStringAttributeValue(IedServer self, DataAttribute* dataAttribute, uint32_t value)
{
    ...
    currentValue = MmsValue_getBitStringAsInteger(dataAttribute->mmsValue);
...
IedServer_updateBooleanAttributeValue(IedServer self, DataAttribute* dataAttribute, bool value)
{
    ...
    currentValue = MmsValue_getBoolean(dataAttribute->mmsValue);
...
IedServer_updateVisibleStringAttributeValue(IedServer self, DataAttribute* dataAttribute, char *value)
{
    ...
    currentValue = MmsValue_toString(dataAttribute->mmsValue);
...
IedServer_updateUTCTimeAttributeValue(IedServer self, DataAttribute* dataAttribute, uint64_t value)
{
    ...
    currentValue = MmsValue_getUtcTimeInMs(dataAttribute->mmsValue);
...
IedServer_updateQuality(IedServer self, DataAttribute* dataAttribute, Quality quality)
{
    ...
    oldQuality = MmsValue_getBitStringAsInteger(dataAttribute->mmsValue);
