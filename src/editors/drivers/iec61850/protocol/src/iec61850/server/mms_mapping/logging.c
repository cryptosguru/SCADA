...
LogInstance_logSingleData(LogInstance* self, const char* dataRef, MmsValue* value, uint8_t flag)
{
    ...
    if (logStorage != NULL) {
        ...
        entryID = LogStorage_addEntry(logStorage, timestamp);

        dataSize = MmsValue_encodeMmsData(value, NULL, 0, false);

        data = (uint8_t*) GLOBAL_MALLOC(dataSize);
...
prepareLogControl(LogControl* logControl)
{
    ...
    dataSet = IedModel_lookupDataSet(logControl->mmsMapping->model, logControl->dataSetRef);
...
enableLogging(LogControl* self)
{
    ...
    if ((self->dataSet != NULL) && (self->logInstance != NULL)) {
        ...
        enabled = MmsValue_getSubElement(self->mmsValue, self->mmsType, "LogEna");
...
getLogInstanceByLogRef(MmsMapping* self, const char* logRef)
{
    ...
    instance = LinkedList_getNext(self->logInstances);
        ...
        if (logControl->enabled == false) {
            ...
            if (logRef == NULL) {
               ...
               if (strcmp(logRef, "") == 0) {
                   ...
                   logInstance = getLogInstanceByLogRef(self, logRef);
...
MmsMapping_setLogStorage(MmsMapping* self, const char* logRef, LogStorage logStorage)
{
    ...
    if (logInstance != NULL) {
        ...
        mmsDomain = MmsDevice_getDomain(self->mmsDevice, domainNameWithIEDName);
