    ...
    case STATE_WAIT_FOR_EXECUTION:
    {
        ...
        if (dynamicCheckResult == CONTROL_RESULT_FAILED) {
            ...
            if (isTimeActivatedControl) {
                ...
                operTm = getOperParameterOperTime(self->oper);
...
ControlObject_initialize(ControlObject* self)
{
    ...
    ctlModelName = StringUtils_createStringInBuffer(strBuf, 4, self->lnName, "$CF$", self->name, "$ctlModel");

    if (DEBUG_IED_SERVER)
        printf("initialize control for %s\n", ctlModelName);

    ctlModel = MmsServer_getValueFromCache(mmsServer,
    ...
    sboClassName = StringUtils_createStringInBuffer(strBuf, 4, self->lnName, "$CF$", self->name, "$sboClass");
    ...
    ctlNumName = StringUtils_createStringInBuffer(strBuf, 4, self->lnName, "$ST$", self->name, "$ctlNum");

    self->ctlNumSt = MmsServer_getValueFromCache(mmsServer, self->mmsDomain, ctlNumName);

    originName = StringUtils_createStringInBuffer(strBuf, 4, self->lnName, "$ST$", self->name, "$origin");
    ...
    if (ctlModel != NULL) {
        ...
        if ((ctlModelVal == 2) || (ctlModelVal == 4)) { /* SBO */
            ...
            sboTimeoutName = StringUtils_createStringInBuffer(strBuf, 4, self->lnName, "$CF$", self->name, "$sboTimeout");
...
Control_processControlActions(MmsMapping* self, uint64_t currentTimeInMs)
{
	...
	CheckHandlerResult checkResult;
...
ControlObject_sendCommandTerminationPositive(ControlObject* self)
{
    ...
    domainId = MmsDomain_getName(self->mmsDomain);
...
checkValidityOfOriginParameter(MmsValue* origin)
{
    ...
    orIdent = MmsValue_getElement(origin, 1);
    ...
    orCat = MmsValue_getElement(origin, 0);

    if (MmsValue_getType(orCat) != MMS_INTEGER)
        return false;

    orCatIntValue = MmsValue_toInt32(orCat);
    ...
    *separator = 0;
    ...
    if (strcmp(varName, "SBOw") == 0) { /* select with value */
        ...
        if (state == STATE_WAIT_FOR_ACTIVATION_TIME) {
            ...
            operTm = getOperParameterOperTime(value);
        ...
        ctlNum = getCancelParameterCtlNum(value);
        origin = getCancelParameterOrigin(value);
