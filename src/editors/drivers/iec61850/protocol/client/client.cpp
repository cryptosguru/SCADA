...
int IEC61850_client_imp::Start(char* Password)
{
    ...
    IsoConnectionParameters parameters = MmsConnection_getIsoConnectionParameters(con);
...
//    IsoConnectionParameters_setLocalApTitle(parameters, NULL, 0);
	...
	if(MmsConnection_connect(con, &mmsError, ServerIPAddress, tcpPort))
	{
	...
	nameList = MmsConnection_getDomainVariableListNames(con, &mmsError, mmsDomain);
	LinkedList_printStringList(nameList);
	LinkedList_destroy(nameList);
	printf("\n");

	nameList = MmsConnection_getDomainVariableNames(con, &mmsError, mmsDomain);
...
void IEC61850_client_imp::pollServer(void)
{
	...
	while((element = LinkedList_getNext(element)) != NULL) 
	{
		...
		typeSpec = MmsConnection_getVariableAccessAttributes(con, &mmsError, mmsDomain, str);
		...
		value = MmsConnection_readVariable(con, &mmsError, mmsDomain, str);
			...
			if(delta < MAX_COMMAND_SEND_TIME && delta >= 0)
			{
				switch(Item[hClient - 1].iec61850_type)
				{
					case MMS_VISIBLE_STRING:
					{
						...
						value_to_write = MmsConnection_readVariable(con, &mmsError, mmsDomain, Item[hClient - 1].spname);
						//value_to_write->type = MMS_VISIBLE_STRING;
						MmsValue_setVisibleString(value_to_write, command_string);

						MmsConnection_writeVariable(con, &mmsError, mmsDomain, Item[hClient - 1].spname , value_to_write);
